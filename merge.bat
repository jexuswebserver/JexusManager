...
"%USERPROFILE%\.nuget\packages\ILRepack\2.0.13\tools\ILRepack.exe" /keyfile:..\JexusManager\JexusManager.snk /t:exe /out:CertificateInstaller.exe CertificateInstaller.exe Microsoft.Web.Administration.dll Mono.Options.dll
@IF %ERRORLEVEL% NEQ 0 PAUSE

del JexusManager.Shared.dll JexusManager.Features.*.dll Ookii.Dialogs.dll System.Net.Http.dll Microsoft.Web.*.dll Crad.Windows.Forms.Actions.dll System.Net.Http.Formatting.dll BouncyCastle.Crypto.dll Mono.Security.dll Vista.Controls.BreadcrumbBar.dll Newtonsoft.Json.dll CheckBoxComboBox.dll MakarovDev.ExpandCollapsePanel.dll Rollbar.dll Octokit.dll
